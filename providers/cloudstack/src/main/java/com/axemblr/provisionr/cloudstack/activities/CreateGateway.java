...
public class CreateGateway extends CloudStackActivity {

    private static final Logger LOG = LoggerFactory.getLogger(CreateGateway.class);

    @Override
    public void execute(CloudStackClient cloudStackClient, Pool pool, DelegateExecution execution) {
        final String gateway = (String) execution.getVariable(CoreProcessVariables.GATEWAY);
        checkNotNull(gateway, "process variable for gateway ({}) is null", CoreProcessVariables.GATEWAY);
        ...
        final String networkId = (String) checkNotNull(execution.getVariable(ProcessVariables.NETWORK_ID));
        ...
        VirtualMachines.waitForVMtoStart(cloudStackClient, gateway);
        LOG.info("Gateway {} has started", gateway);
    }
}
